...
export interface JournalEntry { readonly command: Command; readonly targetTick: number }
export interface CompletedCheckpoint { readonly checkpointId: string; readonly tick: number; readonly state: WorldState; readonly stateHash: string }
...
export class MemoryWorldPersistence implements WorldPersistence {
  ...
  async loadLatestCheckpoint(): Promise<CompletedCheckpoint | undefined> { return this.#checkpoints.at(-1); }
  async loadJournalAfter(tick: number): Promise<readonly JournalEntry[]> { return this.#journal.filter((entry) => entry.targetTick >= tick).map((entry) => structuredClone(entry)); }
  async appendAcceptedCommand(entry: JournalEntry): Promise<void> { this.#journal.push(structuredClone(entry)); }
  async saveCheckpoint(state: WorldState): Promise<CompletedCheckpoint> {
    const checkpoint: CompletedCheckpoint = { checkpointId: randomUUID(), tick: state.tick, state: snapshot(state), stateHash: stateHash(state) };
...
export class PostgresWorldPersistence implements WorldPersistence {
  ...
  async loadLatestCheckpoint(): Promise<CompletedCheckpoint | undefined> {
    const result = await this.pool.query<{ id: string; tick: string; state: WorldState; state_hash: string }>(
      'SELECT id, tick, state, state_hash FROM world_checkpoints WHERE world_id = $1 AND completed = true ORDER BY tick DESC LIMIT 1', [WORLD_ID]);
    const row = result.rows[0];
    return row ? { checkpointId: row.id, tick: Number(row.tick), state: row.state, stateHash: row.state_hash } : undefined;
  }
  async loadJournalAfter(tick: number): Promise<readonly JournalEntry[]> {
    const result = await this.pool.query<{ command: Command; target_tick: string }>(
      'SELECT command, target_tick FROM command_journal WHERE world_id = $1 AND target_tick >= $2 ORDER BY target_tick, accepted_at', [WORLD_ID, tick]);
    return result.rows.map((row) => ({ command: row.command, targetTick: Number(row.target_tick) }));
  }
  async appendAcceptedCommand(entry: JournalEntry): Promise<void> {
    await this.pool.query('INSERT INTO command_journal(id, world_id, target_tick, command) VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO NOTHING', [entry.command.id, WORLD_ID, entry.targetTick, entry.command]);
  }
  async saveCheckpoint(state: WorldState): Promise<CompletedCheckpoint> {
    const checkpoint: CompletedCheckpoint = { checkpointId: randomUUID(), tick: state.tick, state: snapshot(state), stateHash: stateHash(state) };
    const client = await this.pool.connect();
    try {
      ...
      await client.query('INSERT INTO world_checkpoints(world_id, id, tick, schema_version, state, state_hash, completed) VALUES ($1, $2, $3, $4, $5, $6, false)', [WORLD_ID, checkpoint.checkpointId, checkpoint.tick, checkpoint.state.schemaVersion, checkpoint.state, checkpoint.stateHash]);
      await client.query('UPDATE world_checkpoints SET completed = true, completed_at = now() WHERE id = $1', [checkpoint.checkpointId]);
      await client.query('COMMIT');
      return checkpoint;
    } catch (error) { await client.query('ROLLBACK'); throw error; } finally { client.release(); }
  }
  async close(): Promise<void> { await this.pool.end(); }
  private async ensureWorld(client: PoolClient, seed: number): Promise<void> { await client.query('INSERT INTO worlds(id, seed) VALUES ($1, $2) ON CONFLICT (id) DO NOTHING', [WORLD_ID, seed]); }
}

export const createPostgresWorldPersistence = (connectionString: string): PostgresWorldPersistence => new PostgresWorldPersistence(new Pool({ connectionString }));
